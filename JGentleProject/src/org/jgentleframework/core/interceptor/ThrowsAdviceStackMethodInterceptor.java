...
class ThrowsAdviceStackMethodInterceptor implements MethodInterceptor {
		...
		if (objStr != null && !(objStr.length == 1 && objStr[0].isEmpty())) {
			for (String str : objStr) {
				if (!str.isEmpty()) {
					...
					if (obj != null)
						if (ReflectUtils.isCast(ThrowsAdvice.class, obj)) {
							if (!list.contains(obj))
								list.add((ThrowsAdvice) obj);
							addThrowsAdvice((ThrowsAdvice) obj);
						}
						else
							throw new InterceptionException(
									"The registered object of throws advice can not be casted to '"
											+ ThrowsAdvice.class + "'!");
					else {
						if (throwz.required()) {
							throw new RequiredException(
									"The reference of  Throws Advice instance must not be null!");
						}
					}
				}
			}
			...
			if (ReflectUtils.isCast(ParameterizedType.class, type)) {
				...
				if (pType.getRawType().equals(ThrowsAdvice.class)) {
					String annoStrClass = pType.getActualTypeArguments()[0]
							.toString().split(" ")[1];
					Class<? extends Throwable> annoClass = (Class<? extends Throwable>) Class
							.forName(annoStrClass);
					if (annoClass.equals(Throwable.class)) {
						if (!this.throwsAdvices.contains(advice))
							this.throwsAdvices
									.add((ThrowsAdvice<Throwable>) advice);
					}
					else {
	...
	public Object invoke(MethodInvocation invocation) throws Throwable {
		...
		try {
			...
			Definition defMethod = definition.getMemberDefinition(invocation
					.getMethod());
			Throws throwz = defMethod != null
					&& defMethod.isAnnotationPresent(Throws.class) ? defMethod
					.getAnnotation(Throws.class) : null;
			if (throwz != null) {
				Class<?> throwableClass = e.getClass();
				if (throwz.invocation() || runtimeLoading
						|| this.adviceMap.isEmpty()) {
					this.adviceMap.clear();
					findAdviceInstances(throwz);
				}
				if (adviceMap.containsKey(throwableClass))
					adviceMap.get(throwableClass).afterThrowing(invocation, e);
				else
					new InterceptedThrowsAdviceInvocation(invocation, e)
							.proceed();
			}
		}
		return result;
	 ...
	 */
	class InterceptedThrowsAdviceInvocation extends
		 ...
		 *            the throwable
		 */
		public InterceptedThrowsAdviceInvocation(MethodInvocation invocation,
				Throwable throwable) {
		...
		public Object proceed() throws Throwable {
			...
			try {
				index++;
				if (index != throwsAdvices.size())
					throwsAdvices.get(index).afterThrowing(this, throwable);
			}
			finally {
				index--;
			}
